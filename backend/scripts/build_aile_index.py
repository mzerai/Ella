...
build_aile_index.py
-------------------
Build the RAG index from AILE module JSON cells.

Unlike PE (which reads raw Markdown files), AILE content lives directly
...
Usage:
    cd ella/backend
    python scripts/build_aile_index.py

Reads:  data/aile/modules/module_*_cells.json
Writes: data/aile/index/chunks.json
        data/aile/index/embeddings.npy
        data/aile/index/tfidf.pkl
"""
...
AILE_TOPIC_MAP = {
    "aile_00_wakeup": {"topic": "wakeup", "module": "00"},
    "aile_01_demystify": {"topic": "demystify", "module": "01"},
    "aile_02_strategy": {"topic": "strategy", "module": "02"},
    "aile_03_governance": {"topic": "governance", "module": "03"},
    "aile_04_roi": {"topic": "roi", "module": "04"},
    "aile_05_roadmap": {"topic": "roadmap", "module": "05"},
}
...
def extract_chunks_from_module(filepath: str, filename: str) -> list[dict]:
    ...
    topic_info = AILE_TOPIC_MAP.get(module_id)
    if not topic_info:
...
def main():
    modules_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'aile', 'modules')
    index_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'aile', 'index')
    ...
    resources_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'aile', 'resources.json')
    if os.path.exists(resources_path):
        ...
        for i, res in enumerate(resources_list):
            ...
            chunk_content = '\n'.join(content_parts)
            chunk_id = f"aile_resource_{i:02d}"
    ...
    for chunk in all_chunks:
        ...
        source_module_id = [k for k, v in AILE_TOPIC_MAP.items() if v["topic"] == chunk["metadata"]["topic"]][0]
