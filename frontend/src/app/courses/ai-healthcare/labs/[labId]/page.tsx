...
import { 
  getHealthcareWorkshopDetail, 
  evaluateHealthcareResponse,
  type PELabDetail,
  type HealthcareEvalResponse
} from "@/lib/api";
...
const NEXT_SEQUENCES: Record<string, string> = {
  "00_healthcare_ai_use_case_framing_lab": "01_medical_cv",
  "01_medical_cv_diagnostic_support_lab": "02_clinical_nlp",
  "02_clinical_nlp_information_extraction_lab": "03_ethics_privacy",
  "03_healthcare_ai_ethics_anonymization_lab": "04_predictive_risk",
  "04_predictive_risk_decision_support_lab": "05_regulatory_compliance",
  "05_healthcare_ai_regulatory_compliance_tunisia_lab": "final",
};
...
function WorkshopContent() {
  ...
  const [evaluation, setEvaluation] = useState<HealthcareEvalResponse | null>(null);
  ...
  const loadLab = async () => {
    ...
    setError(null);
    try {
      const data = await getHealthcareWorkshopDetail(labId);
      setLab(data);
    } catch (err: any) {
      console.error("Error loading Healthcare workshop:", err);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const resp = await evaluateHealthcareResponse({
        workshop_id: labId,
      ...
      if (resp.score_qualitative === "excellent" || resp.score_qualitative === "good") {
        const completed = JSON.parse(localStorage.getItem("ellaCompletedHealthcareSequences") || "[]");
        ...
        if (currentSeqId !== "unknown" && !completed.includes(currentSeqId)) {
          ...
          localStorage.setItem("ellaCompletedHealthcareSequences", JSON.stringify(completed));
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-6 bg-slate-50 px-4 text-center">
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href="/courses/ai-healthcare" className="bg-teal-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-teal-600/20 hover:scale-105 transition-all">
            Retour au Workshop
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-md z-30 mb-8">
        ...
        <div className="text-center">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
             Atelier Pratique — AI for Healthcare
          </p>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setLang("fr")} className={`px-3 py-1 text-[10px] rounded-full font-black transition-all ${lang === "fr" ? "bg-teal-600 text-white shadow-lg shadow-teal-600/20" : "bg-slate-100 text-slate-400"}`}>FR</button>
          <button onClick={() => setLang("en")} className={`px-3 py-1 text-[10px] rounded-full font-black transition-all ${lang === "en" ? "bg-teal-600 text-white shadow-lg shadow-teal-600/20" : "bg-slate-100 text-slate-400"}`}>EN</button>
        </div>
      </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden">
            ...
            <div className="p-8 md:p-10">
              <div className="prose prose-slate max-w-none prose-headings:text-slate-900 prose-headings:font-black prose-p:text-slate-600 prose-p:font-medium prose-p:leading-relaxed prose-strong:text-teal-600 prose-strong:font-bold">
                <ReactMarkdown>
              ...
              <div className="mt-10">
                <button 
                  ...
                  className="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-teal-600 uppercase tracking-widest transition-colors mb-4"
                >
                  <Sparkles className={`w-4 h-4 ${showHints ? "text-teal-500 fill-teal-500" : ""}`} />
                ...
                {showHints && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-500">
                    {lab.missions[0].hints.map((hint, idx) => (
                      <div key={idx} className="bg-teal-50 border border-teal-100 rounded-2xl p-4 flex gap-3">
                        <span className="text-teal-500 font-black text-xs shrink-0 mt-0.5">{idx + 1}.</span>
                        <p className="text-sm font-bold text-teal-800 leading-relaxed italic">{hint[lang]}</p>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {!evaluation ? (
              <div className="flex flex-col flex-1 gap-4">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest px-2">
                  {lang === "fr" ? "Votre réponse clinique :" : "Your clinical response:"}
                </label>
                <textarea
                  ...
                  placeholder={lang === "fr" ? "Détaillez votre approche, les risques identifiés et les contrôles proposés..." : "Detail your approach, identified risks, and proposed controls..."}
                  className="flex-1 min-h-[300px] w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-6 text-slate-700 font-bold leading-relaxed focus:border-teal-500 focus:ring-0 outline-none transition-all placeholder:text-slate-300 resize-none"
                />
                <button
                  onClick={handleSubmit}
                  disabled={!studentResponse.trim() || isSubmitting}
                  className="w-full bg-teal-600 hover:bg-teal-700 disabled:bg-slate-200 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-teal-600/20 active:scale-95 group uppercase tracking-widest text-xs"
                >
                  {isSubmitting ? (
              ...
              <div className="flex flex-col flex-1 gap-6 animate-in zoom-in-95 duration-500">
                ...
                <div className="flex flex-col gap-3">
                  <button
                    ...
                    className="w-full py-4 rounded-2xl text-xs font-black text-slate-400 hover:text-teal-600 transition-colors uppercase tracking-widest"
                  >
                  ...
                  {(evaluation.score_qualitative === "excellent" || evaluation.score_qualitative === "good") && nextSeqId && (
                    <Link
                      href={nextSeqId === "final" ? "/courses/ai-healthcare" : `/courses/ai-healthcare/sequences/${nextSeqId}`}
                      className="w-full bg-teal-600 hover:bg-teal-700 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-teal-600/20 uppercase tracking-widest text-xs"
                    >
...
export default function HealthcareWorkshopPage() {
  return (
    <ProtectedRoute>
