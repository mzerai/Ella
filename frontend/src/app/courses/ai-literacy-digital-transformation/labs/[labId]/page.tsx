...
const NEXT_SEQUENCES: Record<string, string> = {
  "00_ai_literacy_framing_lab": "01_understand_ai_without_jargon",
  "01_explain_ai_without_jargon_lab": "02_daily_generative_ai_work",
  "02_daily_ai_use_control_lab": "03_practical_prompting_business_work",
  "03_prompting_business_brief_lab": "04_identify_ai_opportunities_department",
  "04_ai_opportunity_canvas_lab": "05_risks_ethics_data_responsible_use",
  "05_responsible_ai_use_lab": "06_from_individual_use_to_digital_transformation",
  "06_digital_transformation_roadmap_lab": "final",
};
...
function LabContent() {
  ...
  const loadLab = async () => {
    ...
    setError(null);
    try {
      const data = await getLiteracyLab(labId);
      setLab(data);
    } catch (err: any) {
      console.error("Error loading Literacy lab:", err);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const resp = await evaluateLiteracyMission(
        labId,
        lab?.missions[0]?.mission_id || "m1",
        studentResponse,
        lang
      );
      setEvaluation(resp);
      ...
      if (resp.score_qualitative === "excellent" || resp.score_qualitative === "good") {
        const completed = JSON.parse(localStorage.getItem("ellaCompletedLiteracySequences") || "[]");
        ...
        if (currentSeqId !== "unknown" && !completed.includes(currentSeqId)) {
          ...
          localStorage.setItem("ellaCompletedLiteracySequences", JSON.stringify(completed));
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-6 bg-slate-50 px-4 text-center">
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href={COURSE_BASE} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-indigo-600/20 hover:scale-105 transition-all">
            Retour au Workshop
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-md z-30 mb-8">
        ...
        <div className="text-center">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
             Lab Pratique — AI Literacy & Transformation
          </p>
            ...
            {!evaluation ? (
              <div className="flex flex-col flex-1 gap-4">
                ...
                <textarea
                  ...
                  placeholder={lang === "fr" ? "Détaillez votre réponse ici..." : "Detail your answer here..."}
              ...
              <div className="flex flex-col flex-1 gap-6 animate-in zoom-in-95 duration-500">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {(evaluation.score_qualitative === "excellent" || evaluation.score_qualitative === "good") && nextSeqId && (
                    <Link
                      href={nextSeqId === "final" ? COURSE_BASE : `${COURSE_BASE}/sequences/${nextSeqId}`}
